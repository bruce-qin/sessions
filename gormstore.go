...
func (st *GormStore) New(r *http.Request, name string) (*gsessions.Session, error) {
	...
	if cookie, err := r.Cookie(name); err == nil {
		...
		if err := st.db.Where("id = ? AND expires_at > ?", session.ID, st.db.NowFunc()).First(s).Error; err != nil {
			return session, nil
		}
...
func (st *GormStore) Cleanup() {
	st.db.Delete(&gormSession{tableName: st.opts.TableName}, "expires_at <= ?", st.db.NowFunc())
